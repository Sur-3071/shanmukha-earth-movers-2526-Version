...
//     let paytotal = 0;
//     let total = 0;
//     alert("driverNames length data entry: " + driverNames.length + ", tripInputs length: " + tripInputs.length);
...
//     const driverNames =
//         document.querySelectorAll('input[name="driverName1[]"]');

//     const tripInputs =
//         document.querySelectorAll('input[name="trips1[]"]');

//     const tripsField =
//         document.getElementById("trips1");

//     const ownTripsField =
//         document.getElementById("hoursnoncompanytractors");
...
//     // let paytotal = 0;
//     // let total = 0;
//     // alert("driverNames length: " + driverNames.length + ", tripInputs length: " + tripInputs.length);
//     // driverNames.forEach((driverName, index) => {
//     //     const trip = tripInputs[index].value || 0;
//     //     const name = (driverName.value.toLowerCase() || "Unknown Driver").trim().replace(/\s+/g, "_");
//     //     if (name.includes("own") || name.includes("సొంత")) {
...
function updateTotalTrips() {
    ...
    driverNames.forEach((driver, index) => {
        ...
        const trip = parseInt(tripInputs[index]?.value) || 0;

        const name = (driver.value || "")
            .trim()
            .toLowerCase()
            .replace(/\s+/g, "_");
    ...
    const totalTrips = document.getElementById("trips");
    if (totalTrips) totalTrips.value = total;

    const ownTrips = document.getElementById("noncompanytractors");
    if (ownTrips) ownTrips.value = ownTotal;
...
function updateTotalTrips1() {
    // alert("updateTotalTrips1 called");
    const driverNames = document.querySelectorAll('input[name="driverName1[]"]');
    const tripInputs = document.querySelectorAll('input[name="trips1[]"]');
    ...
    const totalTrips = document.getElementById("trips1");
    if (totalTrips) totalTrips.value = total;

    const ownTrips = document.getElementById("hoursnoncompanytractors");
    if (ownTrips) ownTrips.value = ownTotal;
...
function tripprice() {
    ...
    const totalTrips =
        parseInt(document.getElementById("trips")?.value) || 0;

    const ratePerTrip =
        parseInt(document.getElementById("jcbtrprate")?.value) || 0;

    const beta =
        parseInt(document.getElementById("beta")?.value) || 0;
    ...
    if (typeof generateOutput === "function") {
        // alert("generateOutput is a function, calling it now.");
        generateOutput();
    }
}
...
function tripprice1() {
    ...
    const totalTrips =
        parseInt(document.getElementById("trips1")?.value) || 0;

    const ratePerTrip =
        parseInt(document.getElementById("trprate1")?.value) || 0;

    document.getElementById("alltrprate").value =
        totalTrips * ratePerTrip;

    if (typeof generateOutput1 === "function") {
        // alert("generateOutput1 is a function, calling it now.");
        generateOutput1();
    }
}
...
function generateOutput() {
    ...
    const driverNames =
        document.querySelectorAll('input[name="driverName[]"]');

    const trips =
        document.querySelectorAll('input[name="trips[]"]');
    ...
    driverNames.forEach((driver, index) => {
        ...
        const trip =
            parseInt(trips[index]?.value) || 0;

            lines.push(`${formattedName} = ${trip}`);
    });
